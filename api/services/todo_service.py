...
class TodoService:
    ...
    def get_todos(self) -> List[Todo]:
        try:
            ...
            self.logger.error(
                f"Unexpected error while fetching todos: {str(e)}")
            raise
    ...
    def create_todo(self, todo: TodoCreate) -> Todo:
        try:
            ...
            self.logger.info(
                f"Successfully created todo with id: {created_todo.id}")
            return created_todo
        except Exception as e:
            self.logger.error(
                f"Unexpected error while creating todo: {str(e)}")
            raise
    ...
    def get_todo_by_id(self, id: int) -> Todo:
        try:
            ...
            try:
                response = self.supabase.table('todos').select(
                    "*").eq('id', id).single().execute()
            except Exception as supabase_error:
                self.logger.warning(
                    f"Supabase error for todo {id}: {str(supabase_error)}")
            ...
            self.logger.error(
                f"Unexpected error while fetching todo {id}: {str(e)}")
            raise
    ...
    def update_todo(self, id: int, todo: TodoUpdate) -> Todo:
        try:
            ...
            self.logger.error(
                f"Unexpected error while updating todo {id}: {str(e)}")
            raise
    ...
    def delete_todo(self, id: int) -> bool:
        try:
            ...
            self.logger.error(
                f"Unexpected error while deleting todo {id}: {str(e)}")
            raise
    ...
    def mark_todo_completed(self, id: int) -> Todo:
        try:
            ...
            self.logger.error(
                f"Error while marking todo {id} as completed: {str(e)}")
            raise
    ...
    def mark_todo_uncompleted(self, id: int) -> Todo:
        try:
            ...
            self.logger.error(
                f"Error while marking todo {id} as uncompleted: {str(e)}")
            raise
